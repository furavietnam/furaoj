...
class Organization(models.Model):
    ...
    monthly_free_credit_limit = models.FloatField(
        default=settings.FURAOJ_MONTHLY_FREE_CREDIT,
        help_text=_('Amount of free credits allocated each month'),
    )
    _pp_table = [pow(settings.FURAOJ_ORG_PP_STEP, i) for i in range(settings.FURAOJ_ORG_PP_ENTRIES)]
    ...
    def calculate_points(self, table=_pp_table):
        ...
        pp = settings.FURAOJ_ORG_PP_SCALE * sum(ratio * pp for ratio, pp in zip(table, data))
    ...
    def max_problems(self):
        return settings.FURAOJ_ORGANIZATION_DEFAULT_MAX_PROBLEMS + (self._active_quota_agg['total_problems'] or 0)

    @cached_property
    def max_storage(self):
        return settings.FURAOJ_ORGANIZATION_DEFAULT_MAX_STORAGE + (self._active_quota_agg['total_storage'] or 0)
...
class Profile(models.Model):
    ...
    display_rank = models.CharField(max_length=10, default='user', verbose_name=_('display rank'),
                                    choices=settings.FURAOJ_DISPLAY_RANKS)
    ...
    def has_enough_solves(self):
        return self.problem_count >= settings.FURAOJ_INTERACT_MIN_PROBLEM_COUNT
    ...
    def can_tag_problems(self):
        if self.allow_tagging:
            if self.user.has_perm('judge.add_tagproblem'):
                return True
            if self.rating is not None and self.rating >= settings.FURAOJ_TAG_PROBLEM_MIN_RATING:
                return True
        return False
    ...
    def calculate_contribution_points(self):
        ...
        count_good_tickets = Ticket.objects.filter(user=self.id, is_contributive=True) \
            .count()
        new_pp = (total_comment_scores + total_blog_scores) * settings.FURAOJ_CP_COMMENT + \
            count_good_tickets * settings.FURAOJ_CP_TICKET
        if new_pp != old_pp:
