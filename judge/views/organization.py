...
class CreateOrganization(PermissionRequiredMixin, TitleMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.has_permission():
            if self.request.user.profile.admin_of.count() >= settings.FURAOJ_ORGANIZATION_ADMIN_LIMIT and \
               not self.request.user.has_perm('spam_organization'):
                return render(request, 'organization/create-limit-error.html', {
                    ...
                    'admin_limit': settings.FURAOJ_ORGANIZATION_ADMIN_LIMIT,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            OrganizationQuota.objects.create(
                ...
                added_problems=packages * settings.FURAOJ_QUOTA_PACKAGE_PROBLEMS,
                added_storage=packages * settings.FURAOJ_QUOTA_PACKAGE_STORAGE,
            )
...
class ProblemCreateOrganization(AdminOrganizationMixin, ProblemCreate):
    ...
    def get(self, request, *args, **kwargs):
        if settings.FURAOJ_QUOTA_ENFORCEMENT_ENABLED and not self.organization.can_create_problem():
...
class OrganizationStorageDashboard(LoginRequiredMixin, TitleMixin, AdminOrganizationMixin,
    ...
    def get_context_data(self, **kwargs):
        ...
        context['cost_chart'] = get_lines_chart(days, {
            _('Cost (thousand vnd)'): [
                round(max(0, c - settings.FURAOJ_MONTHLY_FREE_CREDIT) / sec_per_hour * settings.FURAOJ_PRICE_PER_HOUR, 3)
...
class OrganizationArchivedProblems(LoginRequiredMixin, TitleMixin, AdminOrganizationMixin,
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['archive_retention_days'] = settings.FURAOJ_PROBLEM_ARCHIVE_RETENTION.days
