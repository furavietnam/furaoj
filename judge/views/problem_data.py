...
class ProblemDataView(TitleMixin, ProblemManagerMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.has_perm('judge.create_mass_testcases'):
            ...
            context['testcase_limit'] = settings.FURAOJ_TESTCASE_HARD_LIMIT
            context['testcase_soft_limit'] = settings.FURAOJ_TESTCASE_SOFT_LIMIT

        problem = self.object
        if problem.is_organization_private and problem.organization:
            add_quota_context(problem.organization, context)

        context['quota_warning_suffix'] = settings.FURAOJ_QUOTA_WARNING_SUFFIX
        return context
    ...
    def check_valid(self, data_form, cases_formset):
        ...
        if number_of_cases > settings.FURAOJ_TESTCASE_HARD_LIMIT and \
           not self.request.user.has_perm('judge.create_mass_testcases'):
            error = ValidationError(
                _('Too many testcases, number of testcases must not exceed %s') % settings.FURAOJ_TESTCASE_HARD_LIMIT,
        ...
        if problem.is_organization_private and problem.organization:
            ...
            if settings.FURAOJ_QUOTA_ENFORCEMENT_ENABLED and not org.can_upload_data():
                error = ValidationError(
