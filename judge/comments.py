...
class CommentForm(ModelForm):
    ...
    def clean_body(self):
        ...
        if self.request is not None and not self.request.user.is_staff:
            if len(body) < settings.FURAOJ_COMMENT_MIN_LENGTH:
                raise ValidationError(_('Comment is too short (min %d chars).') % settings.FURAOJ_COMMENT_MIN_LENGTH)
            if len(body) > settings.FURAOJ_COMMENT_MAX_LENGTH:
                raise ValidationError(_('Comment is too long (max %d chars).') % settings.FURAOJ_COMMENT_MAX_LENGTH)

            if settings.FURAOJ_COMMENT_BLACKLIST_TERMS:
                body_casefolded = body.casefold()
                blacklist_casefolded = [term.casefold() for term in settings.FURAOJ_COMMENT_BLACKLIST_TERMS]
        ...
        if self.request is not None and self.request.user.is_authenticated:
            ...
            if profile.mute:
                ...
                raise ValidationError(_('You need to have solved at least %d problems '
                                        'before your voice can be heard.') % settings.FURAOJ_INTERACT_MIN_PROBLEM_COUNT)
            if not self.request.user.is_staff:
                if profile.contribution_points < settings.FURAOJ_COMMENT_MIN_CONTRIBUTION:
                    raise ValidationError(
                        _('You need at least %d contribution points to comment.')
                        % settings.FURAOJ_COMMENT_MIN_CONTRIBUTION,
                    )

                if (settings.FURAOJ_COMMENT_RATE_LIMIT_COUNT is not None and
                        Comment.objects.filter(
                            author=profile,
                            time__gte=timezone.now() - settings.FURAOJ_COMMENT_RATE_LIMIT_WINDOW,
                        ).count() >= settings.FURAOJ_COMMENT_RATE_LIMIT_COUNT):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context['interact_min_problem_count_msg'] = \
                ...
                % settings.FURAOJ_INTERACT_MIN_PROBLEM_COUNT
