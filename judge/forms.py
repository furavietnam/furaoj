...
class ProfileForm(ModelForm):
    ...
    class Meta:
        ...
        if settings.FURAOJ_OFFICIAL_CONTEST_MODE:
            fields.remove('about')
...
class LanguageLimitForm(ModelForm):
    ...
    def clean_time_limit(self):
        ...
        if timelimit and timelimit > settings.FURAOJ_PROBLEM_TIMELIMIT_LIMIT and not has_high_perm:
            raise forms.ValidationError(_('You cannot set time limit higher than %d seconds')
                                        % settings.FURAOJ_PROBLEM_TIMELIMIT_LIMIT,
...
class OrganizationForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if instance is not None:
            ...
            initial['monthly_free_credit_limit'] = settings.FURAOJ_MONTHLY_FREE_CREDIT / 3600
...
class QuotaGrantForm(Form):
    ...
    packages = forms.IntegerField(
        ...
        help_text=_('Each package adds %(storage)s and %(problems)d problems.') % {
            'storage': filesizeformat(settings.FURAOJ_QUOTA_PACKAGE_STORAGE),
            'problems': settings.FURAOJ_QUOTA_PACKAGE_PROBLEMS,
        },
    )
...
class CustomAuthenticationForm(AuthenticationForm, SocialAuthMixin):
    ...
    def confirm_login_allowed(self, user):
        if user.profile.is_banned:
            if user.profile.ban_reason == settings.FURAOJ_CONTEST_CHEATING_BAN_MESSAGE:
...
class ContestForm(ModelForm):
    ...
    def clean(self):
        ...
        if end_time and start_time and \
           (end_time - start_time).days > settings.FURAOJ_CONTEST_DURATION_LIMIT and not has_long_perm:
            raise forms.ValidationError(_('Contest duration cannot be longer than %d days')
                                        % settings.FURAOJ_CONTEST_DURATION_LIMIT,
