...
class ArchiveService:
    ...
    def _request(self, method, path='', **kwargs):
        base = settings.FURAOJ_PROBLEM_ARCHIVE_SERVICE_URL  # read lazily so override_settings works in tests
        if not base:
            raise ArchiveServiceError('the problem archive service is not configured')

        headers = {}
        if settings.FURAOJ_PROBLEM_ARCHIVE_SERVICE_TOKEN:
            headers['Authorization'] = 'Bearer %s' % settings.FURAOJ_PROBLEM_ARCHIVE_SERVICE_TOKEN
